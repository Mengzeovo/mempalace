...
def test_cmd_repair_zero_drawers(mock_config_cls, tmp_path, capsys):
    ...
    with patch("mempalace.cli.open_collection", return_value=mock_col):
        cmd_repair(args)
    out = capsys.readouterr().out
...
def test_cmd_repair_success(mock_config_cls, tmp_path, capsys):
    ...
    mock_chromadb.PersistentClient.return_value = mock_client
    with (
        patch("mempalace.cli.open_collection", return_value=mock_col),
        patch("mempalace.palace.get_collection", return_value=mock_new_col),
...
def test_cmd_compress_no_palace(mock_config_cls, capsys):
    ...
    with (
        patch("mempalace.cli.open_collection", side_effect=Exception("no palace")),
